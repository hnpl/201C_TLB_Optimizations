...
class Design1(Design):
    def __init__(self, num_lanes, page_size_bytes, address_generator, stats_filename = ""):
        super().__init__("design_1", num_lanes, page_size_bytes, address_generator, stats_filename)
    def configure(self):
        # Devices
        ...
        pooled_ptws = PooledPTWsBaseline("ptw_pool", page_table_size = self.page_size_bytes)
        ...
        for i in range(self.num_lanes):
            ...
            ## L1 <-> L2
            l1_tlbs[i].connect_lower_level_device(l2_tlb)
            l2_tlb.connect_higher_level_device(l1_tlbs[i])
        # L2 <-> PTWs
