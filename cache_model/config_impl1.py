...
from objects.ptw import PageTableWalker
...
if __name__ == "__main__":
    ...
    for i in range(num_lanes):
        ...
        ptws.append(PageTableWalker(f"ptw_{i}", 0))
    ...
    for i in range(num_lanes):
        ...
        ## L2 <-> PTW
        l2_tlbs[i].connect_lower_level_device(ptws[i])
        ptws[i].connect_higher_level_device(l2_tlbs[i])
        ## PTW <-> mem
        ptws[i].connect_lower_level_device(backend_memory)
        backend_memory.connect_higher_level_device(ptws[i])
